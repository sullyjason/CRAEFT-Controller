...
const int inputA1 = 3, inputA2 = 4, inputB1 = 5, inputB2 = 6;  // [3,4] for VCA, [5,6] for LRA
#define SLP 7

Servo servo;  // servo motor for pinching force
...
void setup() {
  ...
  scale.set_scale(41692.f);  // this is a random value by Inrak. Should be calibrated later.
  scale.tare();              // reset the scale to 0

  /******* Motor Driver Setup *******/
  servo.attach(9, 1000, 1450);  //2180
...
void loop() {
  ...
  horizontal = analogRead(A0);                    // measure horizontal axis of potentiometer
  vertical = analogRead(A1);                      // measure vertical axis of potentiometer
  proximity = analogRead(A2) - proximity_offset;  // measure proximty of thumb
  ...
  */
  ...
  if (timeSinceLastUpadte333Hz >= timeBetweenUpdates333Hz) {
    ...
    servo.write(setAngle + 1000);  // sets the servo position according to the scaled value
